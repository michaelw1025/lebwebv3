...
@section('content')
        ...
        @component('components.content-title')
                ...
                {{Route('export-team-leader', ['team_leader' => $searchTeamLeader])}}
                @endif
                @endslot
        @endcomponent
        ...
        <form action="{{Route('queries.team-leader')}}" class="mt-2" id="search-team-leader-form" method="GET">
            @csrf
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    <select name="team_leader" id="team-leader-search" class="custom-select {{$errors->has('team_leader') ? 'is-invalid' : ''}}" required>
                        <option value=""></option>
                        @foreach($teamLeaders as $teamLeader)
                        <option {{isset($searchTeamLeader) ? ($searchTeamLeader == $teamLeader->id ? 'selected' : '') : ''}} value="{{$teamLeader->id}}">{{$teamLeader->first_name}} {{$team_leader->last_name}}</option>
                        @endforeach
                    </select>
